class CompressorModule {
  ...
  InitializeElements() {
    this.elements = {
      ...
      compressor_apply_btn: document.getElementById('compressor-apply-btn'),
      compressor_reset_btn: document.getElementById('compressor-reset-btn')
    };
  }
  ...
  AttachEventListeners() {
    ...
    if (this.elements.compressor_apply_btn) {
      this.elements.compressor_apply_btn.addEventListener('click', () => {
        this.ApplyCompression();
      });
    }
    if (this.elements.compressor_reset_btn) {
      this.elements.compressor_reset_btn.addEventListener('click', () => {
  ...
  UpdateInfo() {
    ...
    if (this.elements.compressor_apply_btn) {
      this.elements.compressor_apply_btn.removeAttribute('disabled');
    }
    if (this.elements.compressor_reset_btn) {
      this.elements.compressor_reset_btn.removeAttribute('disabled');
    }
  }

  ApplyCompression() {
    if (!this.compressed_data_url) {
      alert('กรุณารอการบีบอัดให้เสร็จสิ้น');
      return;
    }
    this.app.LoadImage(this.compressed_data_url);
    this.app.history_module.SaveState();
    alert('ใช้งานการบีบอัดเรียบร้อยแล้ว!');
  }
  ...
  OnImageLoaded() {
    ...
    if (this.elements.compressor_apply_btn) {
      this.elements.compressor_apply_btn.setAttribute('disabled', 'disabled');
    }
    if (this.elements.compressor_reset_btn) {
      this.elements.compressor_reset_btn.setAttribute('disabled', 'disabled');
    }
  }
}
